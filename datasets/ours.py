...
test_folder_offset = {"offset_left_0m":0, "offset_left_1m":1, "offset_left_2m":2, "offset_left_4m":4}
...
class OursPixelSource(ScenePixelSource):
    ...
    def __init__(
        ...
        self.pixel_data_config = pixel_data_config
    ...
    def create_all_filelist(self):
        ...
        for t in range(self.start_timestep, self.end_timestep):
            for cam_idx in self.camera_list:
                # 1 cam use
                # if t % self.pixel_data_config.test_image_stride == 0:
                #     img_filepaths.append(
                #         os.path.join(
                #             self.data_path, "test_pic", self.pixel_data_config.test_folder, f"eval_camera0_{(int(t/self.pixel_data_config.test_image_stride)):05d}.png")
                #         )
                # else:
                #     img_filepaths.append(
                #         os.path.join(self.data_path, "train_pic", f"train_camera{cam_idx}_{t:05d}.png")
                #     )
                # 3 cams
                if t % self.pixel_data_config.test_image_stride == 0 and cam_idx == 0:
                    img_filepaths.append(
                        os.path.join(
                            self.data_path, "test_pic", self.pixel_data_config.test_folder, f"eval_camera0_{(int(t/self.pixel_data_config.test_image_stride)):05d}.png")
                        )
        ...
        self.dynamic_mask_filepaths = np.array(dynamic_mask_filepaths) if len(dynamic_mask_filepaths)!=0 else None
        self.sky_mask_filepaths = np.array(sky_mask_filepaths) if len(sky_mask_filepaths)!=0 else None
        self.feat_filepaths = np.array(feat_filepaths) if len(feat_filepaths)!=0 else None
    ...
    def load_calibrations(self):
        ...
        for i in range(self.num_cams):
            ...
            # cam_to_ego = pose_unreal2opencv(cam_to_ego1)
        ...
        #     os.path.join(self.data_path, "train_pic", f"train_camera_extrinsics_{self.start_timestep:06d}.json")
        # )
        
        with open(os.path.join(self.data_path, "train_pic", f"train_camera_extrinsics_{self.start_timestep:06d}.json"), 'r') as file:
            ego_to_world_start = np.array(json.load(file)["transform_matrix"])
        for t in range(self.start_timestep, self.end_timestep):
            # ego_to_world_current = np.loadtxt(
            #     os.path.join(self.data_path, "train_pic", f"train_camera_extrinsics_{t:06d}.json")
            # )
            with open(os.path.join(self.data_path, "train_pic", f"train_camera_extrinsics_{t:06d}.json"),"r") as file:
            ...
            for cam_id in self.camera_list:
                ...
                cam2world_tmp = ego_to_world @ cam_to_egos[cam_id]
                cam2world = pose_unreal2opencv(cam2world_tmp)
        ...
        # 更新测试集的ego pose
        # with open(os.path.join(self.data_path, "test_pic", self.pixel_data_config.test_folder, f"eval_camera_extrinsics_{self.start_timestep:06d}.json"), 'r') as file:
        #     ego_to_world_start1 = np.array(json.load(file)["transform_matrix"])
        for i, filen in enumerate(self.img_filepaths):
            if "eval" in filen:
                car_id = int(filen[-9:-4])
                filen_json = os.path.join(os.path.dirname(filen), f"eval_camera_extrinsics_{int(filen[-9:-4]):06d}.json")
                with open(filen_json,"r") as file:
                    ego_to_world_current1 = np.array(json.load(file)["transform_matrix"])
                    # ego_to_world_current1[0,3] = ego_to_world_current1[0,3] + test_folder_offset[self.pixel_data_config.test_folder]
                ego_to_world1 = np.linalg.inv(ego_to_world_start) @ ego_to_world_current1
                ego_to_worlds[car_id] = ego_to_world1

                cam2world_tmp1 = ego_to_world1 @ cam_to_egos[0]
                cam2world_tmp1[0,3] = cam2world_tmp1[0,3]
                cam2world1 = pose_unreal2opencv(cam2world_tmp1)
                cam_to_worlds[i]=cam2world1
        
        # for t in range(self.start_timestep, self.end_timestep):
        #     # compute ego_to_world transformation
        #     if t % self.pixel_data_config.test_image_stride == 0:
        #         with open(os.path.join(self.data_path, "test_pic", self.pixel_data_config.test_folder, f"eval_camera_extrinsics_{int(t/self.pixel_data_config.test_image_stride):06d}.json"),"r") as file:
        #             ego_to_world_current = np.array(json.load(file)["transform_matrix"])
        #         ego_to_world = np.linalg.inv(ego_to_world_start) @ ego_to_world_current
        #         ego_to_worlds[t] = ego_to_world
        #         for cam_id in self.camera_list:
        #             # transformation:
        #             #   (opencv_cam -> waymo_cam -> waymo_ego_vehicle) -> current_world
        #             cam2world_tmp = ego_to_world @ cam_to_egos[cam_id]
        #             cam2world = pose_unreal2opencv(cam2world_tmp)
        #             cam_to_worlds[t]=cam2world
...
class OursLiDARSource(SceneLidarSource):
    def __init__(
        self,
        ...
        self.start_timestep = start_timestep
        self.end_timestep = end_timestep
        self.lidar_data_config = lidar_data_config
        self.create_all_filelist()
        self.load_data()
    ...
    def create_all_filelist(self):
        ...
        for t in range(self.start_timestep, self.end_timestep):
            lidar_filepaths.append(
                os.path.join(self.data_path, "train_pic", f"train_lidar_{t:05d}.ply")
            )
            # if t%self.lidar_data_config.test_image_stride == 0:
            #     lidar_filepaths.append(
            #         os.path.join(self.data_path, "test_pic", self.lidar_data_config.test_folder, f"test_lidar_{int(t//self.lidar_data_config.test_image_stride):05d}.ply")
            #     )
            # else:
            #     lidar_filepaths.append(
            #         os.path.join(self.data_path, "train_pic", f"train_lidar_{t:05d}.ply")
    ...
    def load_calibrations(self):
        ...
        # # 修改test时候的pose
        # with open(os.path.join(self.data_path, "test_pic", self.lidar_data_config.test_folder, f"eval_camera_extrinsics_{self.start_timestep:06d}.json"), 'r') as file:
        #     ego_to_world_start = np.array(json.load(file)["transform_matrix"])
        # for t in range(self.start_timestep, self.end_timestep):
        #     # ego_to_world_current = np.loadtxt(
        #     #     os.path.join(self.data_path, "ego_pose", f"{t:03d}.txt")
        #     # )
        #     with open(os.path.join(self.data_path, "test_pic", self.lidar_data_config.test_folder, f"eval_camera_extrinsics_{int(t/self.lidar_data_config.test_image_stride):06d}.json"),"r") as file:
        #         ego_to_world_current = np.array(json.load(file)["transform_matrix"])
        #     # compute ego_to_world transformation
        #     if t%self.lidar_data_config.test_image_stride==0:
        #         lidar_to_world = np.linalg.inv(ego_to_world_start) @ ego_to_world_current
        #         lidar_to_worlds[t] = lidar_to_world
    ...
    def load_lidar(self):
        ...
        for t in trange(
            ...
            lidar_info = np.asarray(o3d.io.read_point_cloud(self.lidar_filepaths[t]).points)
            ...
            lidar_origins = torch.zeros(len(lidar_info), 3).float()
...
class OursDataset(SceneDataset):
    dataset: str = "waymo"
    ...
    def __init__(
        ...
        assert self.data_cfg.dataset == "ours"
    ...
    def split_train_test(self):
        if self.pixel_source.num_cams==1:
        ...
        elif self.pixel_source.num_cams==3:
            # 得到测试和训练的indices
            all_files = self.pixel_source.img_filepaths
            train_indices = [idx for idx,filename in enumerate(all_files) if "train" in filename]
            test_indices = [idx for idx,filename in enumerate(all_files) if "eval" in filename]
            # 得到测试时间戳
            train_timesteps = [int(filename[-9:-4]) for filename in all_files if "train" in filename]
            test_timesteps = [int(filename[-9:-4]) for filename in all_files if "eval" in filename]
        logger.info(
